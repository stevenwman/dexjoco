...
class PandaHammerNailGymEnv(MujocoGymEnv):
                ...
                "state": gym.spaces.Dict(
                    {
                        ...
                        "hammer_ori_pose": gym.spaces.Box(-np.inf, np.inf, shape=(7,), dtype=np.float64),
                        "nail_ori_pose": gym.spaces.Box(-np.inf, np.inf, shape=(7,), dtype=np.float64),
                        "table_delta_height": gym.spaces.Box(-np.inf, np.inf, shape=(1,), dtype=np.float64),
                    }
                ),
                "images": spaces.Dict(
                    {
    ...
    def reset(
        ...
        self._model.body_pos[self._model.body("wood").id, 2] = self._wood_body_z0 + self.delta_h
        self._model.body_pos[self._model.body("hammer_body").id, 2] = self._hammer_body_z0 + self.delta_h
        ...
        hammer_orig = np.array(self._data.jnt("hammer_joint").qpos[3:7], dtype=np.float64)
        ...
        if self._randomize_dynamics:
            mass = float(np.random.uniform(self._hammer_mass_mul[0], self._hammer_mass_mul[1]))
    ...
    def _apply_hammer_nail_interaction(self) -> bool:
        ...
        for i in range(ncon):
            ...
            g1 = int(c.geom1)
            g2 = int(c.geom2)
            if (g1 in hammer_set and g2 in nail_set) or (g2 in hammer_set and g1 in nail_set):
                hit = True
                break
