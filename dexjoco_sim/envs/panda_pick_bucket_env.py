...
_SAMPLING_BOUNDS = np.asarray([[-0.20, -0.2], [-0.15, -0.25]])  # x:[-0.20,-0.15], y:[-0.2,-0.25]
...
class PandaPickBucketGymEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        state_space = spaces.Dict(
            {
                ...
                "bucket_ori_pose": spaces.Box(-np.inf, np.inf, shape=(7,), dtype=np.float64),
                "boxed_food_ori_pose": spaces.Box(-np.inf, np.inf, shape=(7,), dtype=np.float64),
        ...
        self.delta_h = 0.0
        self._bucket_bottom_site_ids = [self.model.site(f"bucket_ref_{i}").id for i in (0, 2, 4, 6)]
    ...
    def reset(self, seed=None, **kwargs) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        bucket_orig = np.array(self._data.jnt("bucket_root").qpos[3:7], dtype=np.float64)  # (w,x,y,z)
        ...
        self.bucket_ori_pose = np.concatenate([bucket_xy, [self._bucket_z], bucket_q_new]).astype(np.float64)
        ...
        orig = np.array(self._data.jnt("boxed_food_0_freejoint").qpos[3:7], dtype=np.float64)  # (w,x,y,z)
        ...
        if self._randomize_dynamics:
            frictionloss = float(self._model.dof_frictionloss[self._bucket_dof_id]) * float(
                np.random.uniform(
        ...
        self.env_step = 0
        self._bucket_bottom_z0 = self._data.site_xpos[self._bucket_bottom_site_ids, 2].copy()
