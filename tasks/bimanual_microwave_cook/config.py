...
from dexjoco_sim.envs.panda_bimanual_microwave_cook_env import PandaBimanualMicrowaveCookGymEnv
from tasks.obs_adapters import DexjocoObsAdapter


class TaskConfig(TaskConfigBase):
    proprio_keys = ["tcp_pose", "gripper_pose", "hot_dog_ori_pose", "microwave_ori_pose", "table_delta_height"]
    ...
    def get_environment(self, fake_env=False, render_mode="human", randomize=False, **kwargs):
        env = PandaBimanualMicrowaveCookGymEnv(render_mode=render_mode, randomize=randomize, hz=30, **kwargs)
        if not fake_env:
